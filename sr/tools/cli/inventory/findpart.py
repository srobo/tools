    ...
    for c in args.itemspecs:
        ...
        try:
            ...
            if c in inv.root.types:
                ...
                print("Error: %s is an invalid asset code or part type." % c, file=sys.stderr)
                sys.exit(1)
...
def add_subparser(subparsers):
    parser = subparsers.add_parser('inv-findpart', help='Find the location of a specific item in the inventory.')
    parser.add_argument("-s", "--stat", action="store_true", default=False, dest="asset_stat",
                        help="Show the status of each asset listed based upon the 'condition' field.")
    parser.add_argument("-r", "--relpath", action="store_true", default=False, dest="relpath",
    ...
    parser.add_argument("itemspecs", metavar="ITEM_SPEC", nargs="+",
                        help="Either an SR asset code or part type. " +
                             "The nature of the specifier is auto-detected and asset codes/part types can be mixed.")
