...
def command(args):
    ...
    print(
        'Average time between matches: {}'.format(
            timedelta(seconds=time_between_matches_seconds),
        ),
    )

    float_periods = slots - (rounds * slots_required_for_allvsall)
    vprint(
        'Float time: {} ({} match periods)'.format(
            timedelta(minutes=float_periods * args.match_length),
            float_periods,
        ),
    )
...
def add_subparser(subparsers):
    ...
    parser.add_argument('--arenas', type=int, default=1, help='number of arenas')
    parser.add_argument(
        '--time',
